...
export function HomeTab() {
  ...
  const handleDante = async () => {
    ...
    try {
      ...
      setDanteQuote("Mahaanta ka rasta khamoshi se nikalta hai. Chalte reh.");
    } finally {
  ...
  const handleDoubt = async () => {
    ...
    try {
      ...
      setDoubtOutput("Abhi dimaag kaam nahi kar raha. Thodi der mein aana.");
    } finally {
  ...
  const handleExamDate = () => {
    const d = prompt("Exam ki date batao (YYYY-MM-DD):", examDate || '');
    if (d) setExamDate(d);
  };
  ...
  const buyShield = () => {
    if (user.xp >= 200) {
      ...
      alert("Shield le li bhai! 🛡️");
    } else {
      alert(`${200 - user.xp} XP aur chahiye bhai!`);
    }
  };
    ...
    <div className="p-4 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 shadow-lg border-l-4 border-l-red-600 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          >
            <Sparkles size={12} /> {isDanteLoading ? 'Soch Raha Hai...' : 'Gyaan Maango'}
          </button>
        </div>
        <div className="italic text-zinc-200 text-sm font-medium leading-relaxed">
          {danteQuote ? (
            ...
            '"Yahan average banke aane ki ummeed chhod de. Khasiyat maang, aur shuru karte hain."'
          )}
        </div>
      </div>
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 shadow-lg">
        <div className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2 flex items-center gap-2">
          <Sparkles size={14} className="text-blue-400" /> Doubt Solver
        </div>
        <div className="flex gap-2 mb-2">
          <input 
            ...
            placeholder="E.g. Quantum Entanglement kya hai?" 
      ...
      <div className="bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 rounded-2xl p-5 text-center shadow-lg">
        ...
        {examDate && <div className="text-xs text-zinc-500 mt-2 font-bold">Lakshya: {examDate}</div>}
      </div>
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 shadow-lg">
        <div className="flex justify-between items-center mb-4">
          <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest">⚡ Focus Ke Tools</span>
